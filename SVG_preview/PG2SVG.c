...
float  mmPerStep = 0.092448;//mmPerRev/motorStepsPerRev;
float  steps_per_mm = 21.6338;//10.8169;//8.33333333; //motorStepsPerRev/mmPerRev //    // steps per mm on motor strings
int  mwidth = 722;//1116.0; // space between motor pulleys
float  penWidth = 0.41;
...
void make_line(double x1, double y1, double x2, double y2)
{
	...
	//sprintf( temp_char, "%.3f", x1 );
	//MsvgAddAttribute(son, "x1", temp_char);
	//sprintf( temp_char, "%.3f", y1 );
	//MsvgAddAttribute(son, "y1", temp_char);
	//sprintf( temp_char, "%.3f", x2 );
	//MsvgAddAttribute(son, "x2", temp_char);
	//sprintf( temp_char, "%.3f", y2 );
	//MsvgAddAttribute(son, "y2", temp_char);
...
steps_per_mm = (motorStepsPerRev/mmPerRev); //    // steps per mm on motor strings
	...
	while( !feof(input_file) )
	{
		...
		else if( (strncmp(line,"C05",3) == 0) || (strncmp(line,"C06",3) == 0) )
		{
			...
			//to_cartesian(&x,&y,current_pos[0],current_pos[1]);
			//sprintf( temp_char, "%.3f", x );
			strcpy(points,"");
